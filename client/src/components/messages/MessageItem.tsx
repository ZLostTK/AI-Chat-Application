  ...
  return (
    <div className={`group flex gap-3 ${isUser ? 'self-end flex-row-reverse' : 'self-start'} max-w-[85%] animate-[scale-in_0.2s_ease-out]`}>
      ...
      <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${
        isUser ? 'bg-brand-500 text-white' : 'bg-surface-elevated text-text-primary border-2 border-border'
      }`}>
      ...
      <div className="flex flex-col gap-1 min-w-0 flex-1">
        ...
        <div className={`px-4 py-3 rounded-xl text-sm leading-relaxed break-words ${
          isUser
            ...
            : 'bg-surface-card text-text-primary border border-border rounded-bl-sm shadow-sm'
        }`}>
          {isLoading ? (
