...
export const WelcomeScreen: React.FC<WelcomeScreenProps> = ({ onSendMessage }) => (
  <div className="flex-1 flex flex-col items-center justify-center px-6 py-12 text-center">
    ...
    <h1 className="text-2xl font-bold text-slate-100 mb-2">AI ChatBot</h1>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full max-w-lg">
      {suggestions.map(({ icon: Icon, label, text }) => (
        <button
          ...
          className="flex items-center gap-3 px-4 py-3 bg-surface-elevated border border-border rounded-xl text-sm text-text-secondary hover:text-slate-100 hover:border-brand-400/50 hover:bg-surface-card transition-all text-left"
        >
